...
export default function DeleteKhachHangButton({ id }: { id: number }) {
	...
	return <>
		...
		<ActionModal open={isOpen} title="Xóa khách hàng?" description="Thông tin khách hàng sẽ bị xóa khỏi hệ thống nếu không có dữ liệu nghiệp vụ đang tham chiếu." confirmLabel="Xóa khách hàng" tone="danger" isLoading={isDeleting} error={error} onClose={() => { setIsOpen(false); setError(""); }} onConfirm={() => void handleDelete()} />
	</>;
}
